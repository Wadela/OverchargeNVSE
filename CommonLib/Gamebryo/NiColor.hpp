...
public:
	...
	const float operator[] (UInt32 i) const { return ((float*)&r)[i]; }
	float operator[] (UInt32 i) { return (&r)[i]; }
	...
	static NiColor Lerp(const NiColor& to, const NiColor& from, const float by) {
		return {
			to.r * (1 - by) + from.r * by,
			to.g * (1 - by) + from.g * by,
			to.b * (1 - by) + from.b * by
	...
	NiColor Shifted(const NiColor& target, float intensity) const {
		return {
			r * (1 - intensity) + target.r * intensity,
			g * (1 - intensity) + target.g * intensity,
			b * (1 - intensity) + target.b * intensity
		};
	}
};
...
public:
	...
	inline const float operator[] (UInt32 i) const { return (&r)[i]; }
	inline float operator[] (UInt32 i) { return (&r)[i]; }
	...
	NiColorA Shifted(NiColor target, float intensity) const {
		return {
			r * (1 - intensity) + target.r * intensity,
			g * (1 - intensity) + target.g * intensity,
			b * (1 - intensity) + target.b * intensity,
