...
public:
	...
	bool GetAlphaBlending() const {
		return m_usFlags.Get(ALPHA_BLEND_MASK);
	}

	bool HasAlphaTest() const {
		return m_usFlags.Get(TEST_ENABLE_MASK);
	}

	bool HasAlphaSorter() const {
		return m_usFlags.Get(ALPHA_NOSORTER_MASK);
	}
	...
	void SetAlphaBlending(bool abBlend) {
		m_usFlags.Write(ALPHA_BLEND_MASK, abBlend);
	}

	void SetAlphaTesting(bool abTest) {
		m_usFlags.Write(TEST_ENABLE_MASK, abTest);
	}
