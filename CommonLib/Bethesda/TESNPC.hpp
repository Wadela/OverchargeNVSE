...
public:
	...
	BSFaceGenNiNode*				spHeadBiped;					// 1C4
	BSFaceGenNiNode*				spHeadSkinned;					// 1C8
	NiTexture*						spBodyModTexture;				// 1CC
	UInt16							usLastRaceFaceNum;				// 1D0
	UInt16							unk1D2;							// 1D2
	TESCombatStyle*					pCombatStyle;					// 1D4
	UInt32							hairColor;						// 1D8/20C
	BSSimpleList<BGSHeadPart*>		headPart;						// 1DC/210
	UInt32							impactMaterialType;				// 1E4/218
	UInt32							uiFileOffset;					// 1E8
	TESRace*						pOriginalRace;					// 1EC
	TESNPC*							pFaceNPC;						// 1F0		not set once PlayerRef exists and the target is the Player
