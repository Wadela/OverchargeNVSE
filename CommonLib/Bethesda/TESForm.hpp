...
public:
	enum Type : UInt8 {
		...
		kType_TESObjectCONT,
		kType_TESObjectDOOR,
		kType_IngredientItem,
		kType_TESObjectLIGH,
		kType_TESObjectMISC,
		kType_TESObjectSTAT,
		kType_BGSStaticCollection,
		kType_BGSMovableStatic,
		...
		kType_TESNPC,
		kType_TESCreature,
		kType_TESLevCreature,
		kType_TESLevCharacter,
		kType_TESKey,
		kType_AlchemyItem,
		kType_BGSIdleMarker,
		kType_BGSNote,
		kType_BGSConstructibleObject,
		kType_BGSProjectile,
		kType_TESLevItem,
		kType_TESWeather,
		kType_TESClimate,
		kType_TESRegion,
		kType_NavMeshInfoMap,
		kType_TESObjectCELL,
		kType_TESObjectREFR,
		kType_Character,
		kType_Creature,
		kType_MissileProjectile,
		kType_GrenadeProjectile,
		kType_BeamProjectile,
		kType_FlameProjectile,
		kType_TESWorldSpace,
		kType_TESObjectLAND,
		kType_NavMesh,
		kType_TLOD,
		...
		kType_TESObjectIMOD,
		kType_TESReputation,
		kType_ContinuousBeamProjectile,
		kType_TESRecipe,
		kType_TESRecipeCategory,
		kType_TESCasinoChips,
		kType_TESCasino,
		kType_TESLoadScreenType,
		kType_MediaSet,
		kType_MediaLocationController,
		kType_TESChallenge,
		kType_TESAmmoEffect,
		kType_TESCaravanCard,
		kType_TESCaravanMoney,
		kType_TESCaravanDeck,
		kType_BGSDehydrationStage,
		kType_BGSHungerStage,
		kType_BGSSleepDeprevationStage,
		kType_Count
	};
	...
	// bytes taken by the delta flags for this form, UNRELIABLE, not (always) overriden
	...
	virtual void CreateGroupData(void* apOutGroupFORM, void* apParentGroup);
	// Fills the groupInfo with info valid for the form
	virtual bool Unk_46();
	virtual bool Unk_47();
	...
	enum EnumJIPFormFlags : UInt8 {
		...
		kJIPFormFlag1_ScriptInformed = kJIPFormFlag1_FastTravelInformed | kJIPFormFlag1_CellChangeInformed |
		kJIPFormFlag1_MenuInputInformed,
	...
	bool IsCloned() const {
		return ucModIndex == 0xFF;
	}

	bool IsQuestItem2() const {
		return eFlags & QUEST_ITEM;
	}

	bool IsDisabled() const {
		return eFlags & kFlags_IsPermanent;
	};

	bool IsTaken() const {
		return eFlags & kFlags_Taken;
	}

	bool IsPersistent() const {
		return eFlags & PERSISTENT;
	}

	bool IsTemporary() {
		return eFlags & TEMPORARY;
	}

	bool IsDeleted() const {
		return eFlags & DELETED;
	}

	bool IsDestroyed() {
		return eFlags & kFlags_Destroyed;
	}

	bool IsInteractionDisabled() const {
		return eJIPFormFlags2 & kJIPFormFlag2_NoPCActivation;
	}

	bool IsInventoryObject() const {
		return CdeclCall<char>(0x00481F30, eTypeID);
	}
	...
	__forceinline bool CanContainItems() const {
		return eTypeID == kType_TESObjectCONT || eTypeID == kType_TESNPC || eTypeID == kType_TESCreature;
	}

	__forceinline bool IsActivator() const {
		return eTypeID == kType_TESObjectACTI;
	}
	...
	bool IsWeapon() const {
		return eTypeID == kType_TESObjectWEAP;
	}

	bool IsArmor() const {
		return eTypeID == kType_TESObjectARMO;
	}
